...
function getDepth(tube) {
    if (tube.length === 0)
        return 0;
    let d = 1;
    while (tube.length > d && tube[0] === tube[d])
        d += 1;
    return d;
}
function oneColorTube(tube) {
    return getDepth(tube) === tube.length;
}
function getKey(tubes) {
    return tubes.join("|");
}
function getSetSig(tubes) {
    return tubes.slice().sort().join("|");
}
function getTubesFromKey(key) {
    return key.split("|");
}
function breadthFirstSolve(tubes, timeBudgetMs) {
    const deadline = Date.now() + timeBudgetMs;
    function getAllMoves(n, goal, dictionaries, sets) {
        let newEntries = 0;
        for (let j = 0; j <= n; j++) {
            const i = n - j;
            if (j === 0)
            ...
            for (const k of Object.keys(currentDict)) {
                ...
                for (const c of starts.values()) {
                    for (const v of c) {
                        ...
                        if (oneColorTube(tubesState[v]) || lastEmpty === -1)
                            continue;
                        const m = [v, lastEmpty];
                        const mm = currentDict[k].concat([m]);
                        const tt = doOneMove(tubesState, m);
    ...
    function solveOne(tubes) {
        ...
        const sets = [new Set([getSetSig(tubes)])];
        while (getAllMoves(n, goal, dictionaries, sets) > 0)
            n += 1;
        if (goal < dictionaries.length) {
            for (const dictAtJ of dictionaries[goal]) {
                const keys = Object.keys(dictAtJ);
                if (keys.length > 0)
                    return dictAtJ[keys[0]];
            }
        }
        return null;
    }
 ...
 */
export function solveBoard(board, timeBudgetMs = 8000) {
    const tubes = board.map((tube) => tube.slice().reverse().join(""));
